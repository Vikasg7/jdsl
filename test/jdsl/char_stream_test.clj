...
(deftest char-stream-test
  (testing "create"
    (is (= (cs/create "abc") [(vec "abc") -1]))
    (is (= (cs/create "") [nil -1]))
    (is (= (cs/create nil) [nil -1])))
  (testing "next"
    (is (= (cs/next (cs/create "abc")) [\a [(vec "abc") 0]]))
    (is (= (cs/next (cs/create "")) nil)))
  (testing "eof"
    (is (= (jb/run cs/state (cs/create "abc"))  [[(vec "abc") -1] [(vec "abc") -1]])))
  (testing "peek/1"
